 ...
 * 
 * ESC/POS is the standard command language for thermal printers like Epson, Star, etc.
 */
@Service
@Slf4j
    ...
    public byte[] generateEscPos(Receipt receipt) {
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
            ...
            if (receipt.getVatAmount() != null && receipt.getVatAmount().compareTo(BigDecimal.ZERO) > 0) {
                writeLine(baos, formatRow("MwSt. (" + receipt.getVatRate() + "%):", formatMoney(receipt.getVatAmount(), currency)));
            }
